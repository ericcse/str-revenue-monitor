...
- **Configurable pacing rules** — Columns: **Final Occ. %**, **Target Pctl %**, **Lower Pctl %**, **Wks to Lower**, **Lowest Pctl%**, **Wks to Lowest**. **LY final** (expected market occupancy) is aligned on the **Final Occ. %** scale: the app **linearly interpolates** all five settings between the two surrounding rows (inclusive endpoints match those rows), then applies the time ramp (**Target → Lower** by **Wks to Lower**, then **Lower → Lowest** by stay; **Wks to Lowest** starts the ramp). **LY final** **below** the smallest or **above** the largest Final Occ. % in settings uses that end row **as-is** (no extrapolation). **Wks to Lower** = 0 (or invalid) → one-step Target → Lowest. **Wks to Lowest** = 0 → always Target. **0%** Final Occ. is usually the low anchor; duplicate Final Occ. % in settings → last row wins. Hover **Rec. Pctl %** in the future table for the blended settings used for that week.
